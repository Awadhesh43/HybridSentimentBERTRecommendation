...
class Engine(object):
    ...
    def __init__(self, config):
        ...
        self._metron = MetronAtK(top_k=[1,5,10,20,30,40,50])
    ...
    def evaluate(self, evaluate_data, epoch_id):
        ...
        for k in self._metron.top_k:
            hit_ratio, ndcg = self._metron.cal_hit_ratio(k), self._metron.cal_ndcg(k)
            self._writer.add_scalar('performance/HR@' + str(k), hit_ratio, epoch_id)
            self._writer.add_scalar('performance/NDCG@' + str(k), ndcg, epoch_id)
            print('[Evaluating Epoch {}] HR@{} = {:.4f}, NDCG@{} = {:.4f}'.format(epoch_id, k, hit_ratio, k, ndcg))
